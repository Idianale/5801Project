...
int main() {
	...
	//append this for audit data
	test.appendToString("123");
	test.appendToString("abc");

	//winners for audit/screen
	vector<string> winners;
	winners.push_back("Bob");
	winners.push_back("Rob");

	//losers for audit/screen
	vector<string> losers;
	losers.push_back("Lob");

	//make audit text file
	test.writeToFile(1, 2, 3, winners, losers);

	//write to bash
	test.writeToScreen(1, 100, 2, 3, winners, losers);
