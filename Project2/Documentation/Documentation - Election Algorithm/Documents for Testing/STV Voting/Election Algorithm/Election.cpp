...
//Initalize Election
//This class is basically our "main" where all of our steps on an election take place
...
//Run election is the linear path of running an election. This includes adding votes and determining winners and losers.
...
//Find total candidates that arent declared winners or losers
...
/*
If an STV election is called, this does the countring of the candidates votes. Will be using candidate class in the future but for now a bypass is implmeneted and STVwinnerProtocalB is used.
Finds the winners and losers and lists them in order for the results section of the output. 
*/
...
/*
  STVWinnerProtocol:
    1. Currently not used, needs to be implmented with candidate class in future version of algorthim.
    2. Changes remaining votes in array so that they no longer have that
       candidate, or any previous candidate finalists, as their first choice.
*/
...
/*
  STVLoserProtocolB:
    1. If a new loser is identified with STV loser protocal, the candidate is labeled as a loser and the remaining candidates are listed
    2. Changes remaining votes in array so that they no longer have that
       candidate, or any previous candidate finalists, as their first choice.
*/
     ...
     //   cout << "Votes will now be redistributed based on rankings" << endl;
        for (int bnum = 0; bnum < voteTotal; bnum++) {
            ...
            for (auto i = finalists.begin(); i < finalists.end(); i++) {
                if (bnum >= 0) {
                    if (votes[bnum][*i] == 1) {
                        Audit.append("Before:  Vote" + to_string(bnum) + " reads " + to_string(votes[bnum][0]) +"\n");
                        //       cout << "vote " << bnum << " reads " << votes[bnum][0] << " " << votes[bnum][1]<<endl;
                        adjustVote(votes[bnum]);
                         //      cout << "vote " << bnum << " reads " << votes[bnum][0] << " " << votes[bnum][1]<<endl;
                        voteVals[bnum] = -1;
                        adjustedVote = true;
                        Audit.append("After:  Vote" + to_string(bnum) + " reads " + to_string(votes[bnum][0]) + "\n");
                    }
                    if (adjustedVote){
                        bnum--;
                        }
...
//Currently not used -- will be implmented when candidate and candidates classes are implmeneted
...
/*
 Plurality Protocol:
    1. Counts votes of all ballots in 2D array for each candidate (only looks for the number 1).
    2. If a tie is found, coin toss is used to determine who is ahead in the ranking.
    3. Can be done with multiple seats - returns the one winner or a list of winners in the results string.
